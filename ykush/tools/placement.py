...
PLACE = {
    ...
    # ---- upstream
    'JP1': (47.60, 11.00, 0),            # 0R jumper: USB-C D+ (A6) over D- (A7)
    'U2': (52.00, 14.20, 90),            # upstream ESD
    'R1': (54.50, 10.40, 0),             # CC1 5.1k
    ...
    'C3': (48.40, 16.60, 90),            # VDD5 10u
    'C4': (46.20, 16.60, 90),            # VD33_O 10u
    'C5': (44.40, 16.60, 90),            # VD33 100n
    'C6': (44.20, 12.90, 90),            # VD18_O 10u
    'C7': (42.60, 16.60, 90),            # VD18 100n
    'R5': (54.80, 17.20, 0),             # VBUSM divider top 56k
    'R6': (54.80, 19.00, 0),             # VBUSM divider bottom 100k
    'C8': (54.80, 20.80, 0),             # VBUSM 100n
    'R4': (54.80, 22.60, 0),             # REXT 2.7k

    # ---- MCU (top-left). Its USB link is Full-Speed, so it may use Viagrid vias freely;
...
PORTS = {
    ...
    'P1': dict(sw=('U4', 12.00, 32.00, 180), riset=('R9', 12.00, 29.00, 0), ren=('R10', 8.60, 28.60, 90),
               cin=('C11', 8.60, 32.40, 90), cbulk=('C12', 11.60, 37.60, 0), cout=('C13', 12.00, 35.00, 0),
               esd=('U5', 16.40, 36.60, 90), rled=('R11', 27.60, 42.60, 0), led=('D5', 27.60, 45.40, 0)),
    # port 2 (J3): group left of the socket (its VBUS pin is the left one)
    'P2': dict(sw=('U6', 40.60, 32.80, 180), riset=('R12', 36.60, 30.80, 90), ren=('R13', 40.60, 29.60, 0),
               cin=('C14', 44.00, 30.60, 90), cbulk=('C15', 36.60, 35.40, 90), cout=('C16', 40.60, 36.00, 0),
               esd=('U7', 45.00, 36.60, 90), rled=('R14', 33.80, 42.60, 0), led=('D6', 33.80, 45.40, 0)),
    # port 3 (J4): group right of the socket (the lower-right Viagrid cluster is on its left)
    'P3': dict(sw=('U8', 77.60, 32.00, 0), riset=('R15', 77.60, 29.00, 0), ren=('R16', 76.00, 26.40, 0),
               cin=('C17', 81.20, 32.40, 90), cbulk=('C18', 78.00, 37.60, 0), cout=('C19', 77.60, 35.00, 0),
               esd=('U9', 73.20, 36.60, 90), rled=('R17', 62.75, 42.60, 0), led=('D7', 62.75, 45.40, 0)),
}
