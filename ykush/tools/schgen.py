...
def custom_ic(libid, pins_left, pins_right, pins_top=(), pins_bottom=(), footprint='',
    ...
    w = max(w, wt)
    w = -(-w // 5.08) * 5.08  # pins at +-w/2 must land on the 2.54 grid
    top = h / 2
...
class Sheet:
    ...
    def write(self, path):
        doc = [S('kicad_sch'), [S('version'), S('20260101')], [S('generator'), S('eeschema')],
    ...
    def write_project_lib(self, prefix, path):
        """Write the custom symbols (lib ids starting with prefix:) as a .kicad_sym library."""
        import copy
        syms = []
        for lid, s in self.libs.items():
            if lid.startswith(prefix + ':'):
                s = copy.deepcopy(s)
                s[1] = lid.split(':', 1)[1]
                syms.append(s)
        doc = [S('kicad_symbol_lib'), [S('version'), S('20220914')], [S('generator'), S('ykush_vg_schgen')]] + syms
        with open(path, 'w') as f:
            f.write(dump(doc) + '\n')
